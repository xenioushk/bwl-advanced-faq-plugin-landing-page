...
jQuery(function ($) {
    ...
    $('a.scrollto').on('click', function (e) {
        ...
        var $this = $(this),
                target = this.hash,
                $highlight_status = $this.hasClass('highlight');
        e.preventDefault();
        
        var offset = $('.header-wrapper').outerHeight();
        
        $('body').scrollTo(target, 3000, {offset: -106, 'axis': 'y', onAfter: function () {
...
//            $(".filter").not('.filter[filter-item="'+value+'"]').addClass('hidden');
    ...
    if ($(".gallery-carousel").length) {
        ...
        $gallery_carousel.each(function () {
            ...
            if ($this.attr('data-lazy') && !isNaN($this.data('lazy'))) {

                lazy_val = $this.data('lazy');
